...
let craftingRecipes = $derived(
    liveQuery(() => db.craftingRecipes.where("usedItemsFullIds").equals($page.params.item).toArray(), {}, $page.params.item),
);

let furnaceRecipes = $derived(
    liveQuery(
        () => db.furnaceRecipes.where("usedItem.fullId").equals($page.params.item).toArray(),
        { initialValue: [] },
        $page.params.item,
    ),
);
